...
```r
b <- filter(gapminder, country == c("Rwanda", "Afghanistan"))
knitr::kable(b)
```
...
```r
c <- filter(gapminder, country == c("Rwanda")) 
knitr::kable(c)
```
...
```r
d <- filter(gapminder, country == c("Afghanistan")) 
knitr::kable(d)
```
...
```r
e <- filter(gapminder, country %in% c("Rwanda", "Afghanistan"))
knitr::kable(e)
```
